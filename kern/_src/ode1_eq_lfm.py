...
class Ode1_eq_lfm(Ode1_lfm):
    ...
    def __init__(self, output_dim=1, decay=1, variance=1, lengthscale=1, name='Ode1_eq_lfm'):
        ...
        k_fx = sym.sqrt(sym.pi)*scale/2*sym.exp(decay*lengthscale/2)**2*sym.exp(-decay*(z_0-x_0)) * (sym.erf(z_0-x_0/lengthscale-decay*lengthscale/2) + sym.erf(x_0/lengthscale+decay*lengthscale/2))
        k_xx = scale**2*(    h(x_0, z_0, decay, decay, lengthscale)
                           + h(z_0, x_0, decay, decay, lengthscale)
                        )

        super(Ode1_eq_lfm, self).__init__(input_dim, k_ff, k_fx, k_xx, output_dim, name)
